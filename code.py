...
def session_logout(req, page, link):
    ...
    form_build_val = re.search(
        '"form_build_id" id="(.*?)"', page.text).group(1)
    form_token_val = re.search(
        '"form_token".*? value="(.*?)"', page.text).group(1)
...
def codechef_login(req, link):
    ...
    if re.search('Logout', page.text):
        ...
        raise SystemExit(
            'Login messed up at some point, please check you ACCOUNT Details!')
